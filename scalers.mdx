...
Name | Type | Description | Default
---- | ---- | ----------- | -------
`skipna` | <code>[bool](#bool)</code> | If True, exclude NaN values when computing statistics. When False (default), only a leading run of NaNs is supported; an interior one leaves the statistics unspecified, so pass True for that. | <code>False</code>
