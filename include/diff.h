...
{
  template <typename T>
  void Differences(const T *data, int n, int d, T *out)
  {
    seasonal::Difference(data, n, out, d);
  }

  template <typename T>
  inline bool IsConstant(const T *data, int n)
  {
    for (int i = 1; i < n; ++i)
    {
      if (data[i] != data[0])
  ...
  template <typename T>
  void NumDiffs(const T *x, indptr_t n, T *out, int max_d)
  {
  ...
  void NumSeasDiffs(const T *x, indptr_t n, T *out, int period, int max_d)
  {
    ...
    while (do_diff && d < max_d)
    {
      ...
      {
        do_diff = seasonal::SeasHeuristic(x_vec.data() + d * period, n - d * period,
                                          period) > threshold;
      }
      else
      {
