...
py::array_t<T> RollingOp(Func f, const py::array_t<T> data, int window_size, int min_samples, Args... args)
{
    ...
    f(data.data(), data.size(), out.mutable_data(), window_size, min_samples, std::forward<Args>(args)...);
    return out;
}

template <typename T>
py::array_t<T> RollingMean(const py::array_t<T> data, int window_size, int min_samples)
{
    return RollingOp(rolling::MeanTransform<T>, data, window_size, min_samples);
}

template <typename T>
py::array_t<T> RollingStd(const py::array_t<T> data, int window_size, int min_samples)
{
    return RollingOp(rolling::StdTransform<T>, data, window_size, min_samples);
}

template <typename T>
py::array_t<T> RollingMin(const py::array_t<T> data, int window_size, int min_samples)
{
    return RollingOp(rolling::MinTransform<T>, data, window_size, min_samples);
}

template <typename T>
py::array_t<T> RollingMax(const py::array_t<T> data, int window_size, int min_samples)
{
    return RollingOp(rolling::MaxTransform<T>, data, window_size, min_samples);
}

template <typename T>
py::array_t<T> RollingQuantile(const py::array_t<T> data, int window_size, int min_samples, T p)
{
    return RollingOp(rolling::QuantileTransform<T>, data, window_size, min_samples, p);
}

template <typename T, typename Func, typename... Args>
py::array_t<T> SeasonalRollingOp(Func f, const py::array_t<T> data, int season_length, int window_size, int min_samples, Args... args)
{
    py::array_t<T> out(data.size());
    f(data.data(), data.size(), out.mutable_data(), season_length, window_size, min_samples, std::forward<Args>(args)...);
    return out;
}
...
template <typename T>
void init_roll_fns(py::module_ &m)
{
