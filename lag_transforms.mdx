...
`skipna` | <code>[bool](#bool)</code> | If True, exclude NaN values from calculations. When False (default), only a leading run of NaNs is supported; an interior one leaves the result unspecified, so pass True for that. | <code>False</code>
...
`skipna` | <code>[bool](#bool)</code> | If True, exclude NaN values from calculations using forward-fill behavior. When False (default), only a leading run of NaNs is supported; an interior one leaves the result unspecified, so pass True for that. | <code>False</code>
