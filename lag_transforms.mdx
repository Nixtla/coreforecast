...
`skipna` | <code>[bool](#bool)</code> | If True, exclude NaN values from calculations. When False (default), an interior NaN makes `transform` NaN from it on, but `update` recomputes from the last window and recovers once the NaN leaves it, so it supports only a leading run of NaNs; pass True for anything else. | <code>False</code>
...
---- | ---- | ----------- | -------
`lag` | <code>[int](#int)</code> | Number of periods to offset by before applying the transformation | *required*
`skipna` | <code>[bool](#bool)</code> | If True, exclude NaN values from calculations. When False (default), a leading run of NaNs is kept and the statistic computed over what follows it; a later NaN makes the result NaN from it on, in `transform` and `update` alike. | <code>False</code>
...
`skipna` | <code>[bool](#bool)</code> | If True, exclude NaN values from calculations using forward-fill behavior. When False (default), a leading run of NaNs is kept and the statistic computed over what follows it; a later NaN makes the result NaN from it on, in `transform` and `update` alike. | <code>False</code>
